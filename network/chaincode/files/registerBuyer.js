...
rl.question('Enter enrollment id: ', async (eid) => {
    ...
    try {
        ...
        const adminIdentity = await wallet.get('buyer_admin');
        if (!adminIdentity) {
            console.log(`An identity for the buyer_admin user ${eid} does not exist in the wallet`);
        ...
        const adminUser = await provider.getUserContext(adminIdentity, 'buyer_admin');
        ...
        console.log(`Successfully registered and enrolled buyer_admin use ${eid} and imported it into the wallet`);

    } catch (error) {
        console.error(`Failed to register buyer ${eid}: ${error}`);
        process.exit(1);
    }
    rl.close();
});